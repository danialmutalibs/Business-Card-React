import React from 'react'
import './MainContent.css'
import me from './assets/me.png'
import facebook from './assets/facebook.svg'
import linkedin from './assets/linkedin.svg'
import github from './assets/github.svg'
import mail from './assets/Mail.svg'
import youtube from './assets/youtube.svg'

export default function MainContent(){
    return (
        <>
            <div className='container'>
                <div className='img-container'>
                <img className='main-img' src={me} alt="me" />
                </div>
                <div className='sub-container'>
                    <h2 className='name'>Muhammad Danial Mutalib</h2>
                    <h4 className='role'>Full-Stack Developer</h4>
                    <div className='connect'>
                        <a className='mail' href="mailto:danialmutalib.s@gmail.com"><img src={mail} alt="mail" /> Email</a>
                        <a className='linkedin' href="https://www.linkedin.com/in/danial-mutalib-387580194/" target='_blank'><img src={linkedin} alt="" />LinkedIn</a>
                    </div>
                    <div className='about'>
                        <h3>About</h3>
                        <p>I am an aspiring full-stack developer. I am constantly learning new technologies and knowledge related to my future career. </p>
                    </div>
                    <div className='interest'>
                        <h3>Interests</h3>
                        <p> Music. Theatre. Technology. I love connecting with new people, give me a shout at Minhvy.ha@outlook.com.</p>
                    </div>
                </div>
                <div className='footer'>
                    {/* TO BE USED IN THE FUTURE */}
                    {/* <a href="https://www.facebook.com/minhvy.ha.75/"><img className='facebook' src={facebook} alt="facebook" /></a>
                    <a href="https://leetcode.com/minhvy828/"><img className='leet' src="data:image/png;base64," /></a>
                    <a href="https://www.youtube.com/channel/UC6kYs1bW0j273GneHxFyPFQ"><img className='youtube' src={youtube} alt="youtube" /></a> */}
                    <a href="https://github.com/danialmutalibs"><img src={github} alt="github" /></a>
                </div>
            </div>
        </>
    )
}
